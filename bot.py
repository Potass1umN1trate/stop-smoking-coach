...
async def send_next_motivation(user_id, bot):
    ...
    hardness = user[3]
    prompt = (
       f'''Дай одную мотивационную цитату, чтобы достичь следующей цели:"{goal}". Степень жесткости мотивации должна быть {hardness} из 21. Ответ предоставь в JSON в следующем формате {{"motivation":"<текст мотивации на русском языке>"}}.'''
    )
    try:
